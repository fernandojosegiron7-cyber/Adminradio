...
export default async function handler(req, res) {
  ...
  const requested = Number(req.query?.size || 512);
  const source = requested <= 192 ? config.pwaIcon192Data : config.pwaIcon512Data;
  const parsed = parseDataUrl(source) || parseDataUrl(config.logoData);
