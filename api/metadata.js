...
export default async function handler(req, res) {
  ...
  if (!config.metadataUrl) {
    return safeJson(res, 200, {
      ...
      artist: config.fallbackArtist || config.brandName,
      artwork: config.logoData || ''
    });
  }
  try {
    if (/api\.zeno\.fm\/mounts\/metadata\/subscribe\//i.test(config.metadataUrl)) {
      return safeJson(res, 200, { title: config.fallbackTitle, artist: config.fallbackArtist || config.brandName, artwork: config.logoData || '', liveSource: 'zeno-eventsource' });
    }
